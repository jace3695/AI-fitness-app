...
export default function MemoryPractice({ attempt, records, easy, disabled, onChange }: { attempt: Attempt; records: Attempt[]; easy: boolean; disabled: boolean; onChange: (patch: Partial<DrawingDocument>) => void }) {
  ...
  return <div className="space-y-4" role="region" aria-label="기억해서 다시 그리기">
    ...
    <label className="block text-sm">기억나는 특징 · 말해도 괜찮아요<textarea aria-label="기억나는 특징 · 말해도 괜찮아요" className="drawing-input mt-2 w-full" disabled={disabled} maxLength={500} value={state.recalled} onChange={e => patch({ recalled: e.target.value })} placeholder="예: 큰 얼굴 위에 작은 귀 두 개" /></label>
    {phase === 'compare' && <label className="block text-sm">다시 보고 보완할 한 곳<textarea aria-label="다시 보고 보완할 한 곳" className="drawing-input mt-2 w-full" disabled={disabled} maxLength={500} value={state.compared} onChange={e => patch({ compared: e.target.value })} placeholder="예: 귀 위치를 다시 봤어요. 오른쪽 귀만 조금 옮길래요." /></label>}
