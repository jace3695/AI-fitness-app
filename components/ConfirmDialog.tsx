...
export default function ConfirmDialog({ open, title, description, children, confirmLabel = '삭제', busy = false, onCancel, onConfirm }: {
  ...
  return <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-950/45 p-4" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onCancel(); }}>
    <div ref={ref} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={`${id}-title`} aria-describedby={`${id}-description`} className="max-h-[85dvh] w-full max-w-md overflow-y-auto rounded-3xl bg-white p-5 text-gray-900 shadow-xl">
      ...
      {children}
      <div className="mt-5 grid grid-cols-2 gap-3"><button type="button" disabled={busy} onClick={onCancel} className="min-h-11 rounded-xl bg-gray-100 font-bold disabled:opacity-50">취소</button><button type="button" disabled={busy} onClick={onConfirm} className="min-h-11 rounded-xl bg-red-600 font-bold text-white disabled:opacity-50">{busy ? '처리 중…' : confirmLabel}</button></div>
    </div>
  </div>;
}
