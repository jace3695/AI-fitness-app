...
export default function KanaStarter() {
  ...
  return <section className={styles.focus}>
    ...
    <LearningCompanion hidden={!showCompanion}
      action={mode === "done" || (mode === "quiz" && picked === current) ? "celebrate" : mode === "quiz" && picked ? "encourage" : "explain"}
      motionKey={`${group.id}:${mode}:${letterIndex}:${picked ?? ""}`}>
      {mode === "done" ? "한 묶음을 해냈어요! 인사말을 배우러 가도 좋고, 다음 글자를 만나도 좋아요." : mode === "learn" ? "먼저 소리를 듣고 한 번 따라 해봐요. 전부 외우려고 애쓰지 않아도 돼요." : picked === current ? "맞았어요! 조금씩 익숙해지고 있어요." : picked ? "괜찮아요. 설명을 보고 함께 다시 골라봐요." : "방금 배운 글자를 찾아볼까요? 틀려도 다시 고르면 돼요."}
    </LearningCompanion>
