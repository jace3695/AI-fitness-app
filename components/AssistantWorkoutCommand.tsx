...
import { describeWorkoutCardio, nextWorkoutCardioSnapshot } from '@/lib/assistant-workout-cardio-command';
...
export function AssistantWorkoutReview({ proposal, onChanged, ownerId, initiallyAttempted = false, onAttempt, onSettled }: {
  ...
  return <section aria-label={title} className="mt-3 rounded-2xl border border-violet-200 bg-violet-50 p-3 text-gray-800">
    ...
    {proposal.change?.kind==='feedback' ? <><p className="mt-2 text-sm">변경 전: {describeWorkoutFeedback(proposal.expected)}</p><p className="mt-2 text-sm">변경 후: {describeWorkoutFeedback(nextWorkoutFeedbackSnapshot(proposal.expected,proposal.change))}</p><p className="mt-2 text-xs">직접 말한 항목만 저장합니다. 일부 완료·중단은 완료 표시를 해제하고, 통증 부위 입력은 통증 있음으로 기록합니다. 실제 세트와 다른 날짜는 보존합니다.</p></> : proposal.change ? <>
      ...
      <p className="mt-2 text-xs text-gray-600">오늘의 완료 사실을 기록합니다. 운동 종류·세트·횟수·통증은 운동 화면에서 입력해 주세요.</p>
    </>}
    <p className="mt-2 text-xs text-gray-600">확인 화면은 15분간 유효하며 같은 탭에서 새로고침하면 복구됩니다.</p>
...
export function AssistantWorkoutHistory() {
  ...
  return <section aria-label="운동 실행 이력 목록"><p className="mt-2 text-sm leading-6 text-gray-600">연이 대화·빠른 명령에서 확인하고 저장한 운동 완료·유산소 기록 이력입니다.</p>
