...
export function AssistantDietReceipt({ receipt, onChanged }: { receipt: DietCommandReceipt; onChanged?: () => void | Promise<void> }) {
  ...
  return <article aria-label="식단 실행 이력" className="rounded-2xl border border-violet-100 bg-white p-4 text-sm text-gray-800">
    ...
    <dl className="mt-3 space-y-1"><div><dt className="inline font-bold">변경 전: </dt><dd className="whitespace-pre-wrap break-all">{describe(saved.before_values, saved.change, saved.record_date)}</dd></div><div><dt className="inline font-bold">변경 후: </dt><dd className="whitespace-pre-wrap break-all">{describe(saved.after_values, saved.change, saved.record_date)}</dd></div></dl>
...
export function AssistantDietReview({ proposal, onChanged, ownerId, initiallyAttempted = false, onAttempt, onSettled }: {
  ...
  return <section aria-label="식단 기록 확인" className="mt-3 rounded-2xl border border-violet-200 bg-violet-50 p-3 text-gray-800">
    ...
    <p className="mt-2 whitespace-pre-wrap break-all text-sm">변경 전: {describe(proposal.expected, proposal.change, proposal.date)}</p><p className="mt-2 whitespace-pre-wrap break-all text-sm">저장할 내용: {describe(dietNextSnapshot(proposal.expected, proposal.change, proposal.date), proposal.change, proposal.date)}</p>
    <p className="mt-2 text-xs text-gray-600">{proposal.change.kind === 'time' ? '마지막 식사 시각과 공복 계산 시작 시각을 함께 저장합니다. 18:30 이전 여부는 입력 시각에 따라 반영하며, 공복 달성 여부는 직접 기록합니다.' : proposal.change.kind === 'meal' ? `해당 끼니의 ${proposal.change.field === 'protein' ? '식품에 들어 있는 단백질 g(음식 자체의 무게가 아님)' : '조리된 밥의 무게 g'}을 입력한 총량으로 바꿉니다. 다른 식사와 보충제 기록은 유지합니다.` : proposal.change.kind === 'water' ? '오늘 마신 물의 총량을 저장합니다. 기존 수분량에 더하지 않습니다.' : '입력한 내용을 기존 식단 메모 뒤에 추가합니다.'} 확인 화면은 15분간 유효하며 같은 탭에서 새로고침하면 복구됩니다.</p>
