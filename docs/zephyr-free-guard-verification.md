...
> 2026-09-16 후속: 제공된 보고서에서 Chirp3-HD 244자·0원을 확인한 뒤, 9월 월 승인과 Preview 전용 답변 읽기를 활성화했다. 일반 경로 1회·38자·5.616초 생성·재생 및 다시 듣기 후 추가 차감 없음을 확인했다. 아래는 안전장치 구현 당시 기록이며 [최신 상태](zephyr-answer-playback.md)를 우선한다.

실행 순서 **3번의 서버 안전장치 준비**다. 음성 재연결 전체 완료가 아니다. 실제 음성 활성화·가계부/빠른 명령 재생 연결·iPhone 확인은 결제 검증 후 진행한다. `FREE_MODE=true`는 그대로다.

후속 요청: 사용자가 앱에서 실제 사용량을 발생시키는 소수의 검증과 전용 DB 설정을 승인했다. 별도의 [최대 3회 실제 생성·재생 성공 기록](./zephyr-three-call-probe.md)에 결과를 남겼다. 3회·183자 모두 Google HTTP 200과 앱 재생 완료를 확인했으며, 월별 무료 사용 조건이나 실제 청구 금액 확인을 뜻하지 않는다.
