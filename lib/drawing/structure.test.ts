...
import {structureState,structureEligible,useAnalysis} from './structure.ts';
...
test('D46 copies saved analysis without editing source or assembly strokes; wrong sources rejected',()=>{
 const a=attempt(46),source=attempt(45,1);source.document.strokes=[structuredClone(ink)];a.document.strokes=[structuredClone(ink)];const before=structuredClone(source);a.document={...a.document,...useAnalysis(a.document,source)};assert.deepEqual(source,before);assert.equal(a.document.example.id,source.document.example.id);assert.equal(a.document.structure!.analysis.length,1);assert.equal(a.document.strokes.length,1);a.document.structure!.analysis[0].points[0][0]=1;assert.equal(source.document.strokes[0].points[0][0],100);assert.equal(parseAttempt(a).document.references[0],source.id);assert.throws(()=>useAnalysis(attempt(43).document,source));assert.throws(()=>useAnalysis(a.document,attempt(45)));source.document.tool='paper';assert.throws(()=>useAnalysis(a.document,source));
});
