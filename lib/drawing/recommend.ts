import {originalEligible} from "./original.ts";
import { identityEligible } from "./identity.ts";
...
export function recommend(pack: Pack, history: Attempt[], requiredExamples = 2): Recommendation | null {
  ...
  if (!originalEligible(doc)) return {...base,exampleId:doc.example.id,reason:"마지막 비교에서 내 원본·모음과 역할·특징을 확인해요. 일부만 저장하고 이어 해도 돼요."};
  if (!identityEligible(doc)) return { ...base, exampleId: doc.example.id, reason: "마지막 비교에서 기준 특징 두 개와 그림 모음을 확인해요." };
  ...
  const confirmed = history.filter(a => a.status === "completed" && !a.document.short && a.document.check === "independent" && !a.document.difficulty && a.document.lesson.id === current.id && recallEligible(a.document) && variationEligible(a.document) && structureEligible(a.document) && gestureEligible(a.document) && identityEligible(a.document) && originalEligible(a.document));
  const distinct = new Set(confirmed.map(a => a.document.example.id));
  if (distinct.size < ((current.originalPractice || current.identityPractice && current.identityPractice !== "draw") ? 1 : Math.max(2, requiredExamples))) return { ...base, exampleId: current.examples.find(e => !distinct.has(e.id))?.id ?? base.exampleId, reason: "스스로 해봤다고 체크했어요. 다른 예제에서도 같은 목표를 확인해요." };
  const next = available.find(l => pack.lessons.indexOf(l) > pack.lessons.indexOf(current));
  return next ? { lessonId: next.id, exampleId: next.examples[0].id, help: next.help as Help, short: false, reason: current.originalPractice ? "내 캐릭터의 이번 목표를 스스로 확인했어요. 다음 목표로 이어 만들어요." : "서로 다른 예제에서 스스로 확인했어요. 다음 목표를 살펴봐요. 완료 횟수로 실력을 채점하지 않아요." }
...
export function confirmedStage(pack: Pack, history: Attempt[], stage: number) {
  ...
  const evidence = history.filter(a => a.document.lesson.id === capstone.id && a.status === "completed" && a.document.check === "independent" && !a.document.short && !a.document.difficulty && recallEligible(a.document) && variationEligible(a.document) && structureEligible(a.document) && gestureEligible(a.document) && identityEligible(a.document) && originalEligible(a.document));
  return new Set(evidence.map(a => a.document.example.id)).size >= ((capstone.originalPractice || capstone.identityPractice === "poses") ? 1 : 2);
}
