...
export function recommend(pack: Pack, history: Attempt[], requiredExamples = 2): Recommendation | null {
  ...
  const confirmed = history.filter(a => a.status === "completed" && !a.document.short && a.document.check === "independent" && !a.document.difficulty && a.document.lesson.id === current.id);
...
export function feedback(attempt: Attempt, pack: Pack, history: Attempt[]) {
  ...
  return [
    ...
    { title: "현재 학습 단계", body: `${d.lesson.stage}단계 · ${HELP_LABELS[d.usedHelp]}. 확인 근거: 사용자 자기확인. 그림 분석은 받지 않았어요.` },
  ];
}
...
export function confirmedStage(pack: Pack, history: Attempt[], stage: number) {
  ...
  const evidence = history.filter(a => a.document.lesson.id === capstone.id && a.status === "completed" && a.document.check === "independent" && !a.document.short && !a.document.difficulty);
