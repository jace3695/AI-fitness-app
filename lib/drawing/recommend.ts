...
export function recommend(pack: Pack, history: Attempt[], requiredExamples = 2): Recommendation | null {
  ...
  const confirmed = history.filter(a => a.status === "completed" && !a.document.short && a.document.check === "independent" && !a.document.difficulty && a.document.lesson.id === current.id && recallEligible(a.document) && variationEligible(a.document));
...
export function confirmedStage(pack: Pack, history: Attempt[], stage: number) {
  ...
  const evidence = history.filter(a => a.document.lesson.id === capstone.id && a.status === "completed" && a.document.check === "independent" && !a.document.short && !a.document.difficulty && recallEligible(a.document) && variationEligible(a.document));
