...
export type DietDaySnapshot = DietMealSnapshot;
export type DietCommandChange = { kind: 'water'; totalMl: number } | { kind: 'memo'; text: string } | DietMealChange;
...
function isDietSnapshot(value: unknown, meal = false): value is DietDaySnapshot {
  return object(value) && Object.keys(value).every(key => key === 'record' || key === 'water' || (meal && Object.hasOwn(DIET_MEAL_STORE_KEYS, key)))
    && (!Object.hasOwn(value, 'record') || object(value.record))
    && (!Object.hasOwn(value, 'water') || waterValue(value.water)) && (!meal || isDietMealSnapshot(value));
}
export function dietDaySnapshot(state: Record<string, unknown>, date: string, change?: DietCommandChange): DietDaySnapshot {
  const snapshot: Record<string, unknown> = {};
  const keys = { record: DIET_RECORD_KEY, water: DIET_WATER_KEY, ...(change?.kind === 'meal' ? DIET_MEAL_STORE_KEYS : {}) };
  ...
  if (!isDietSnapshot(snapshot, change?.kind === 'meal')) throw new Error('오늘 식단 기록을 확인하지 못했습니다. 식단 화면에서 확인해 주세요.');
  return snapshot;
}
export function isDietChange(value: unknown): value is DietCommandChange {
  if (isDietMealChange(value)) return true;
...
export const isDietRecordIntent = (message: string) => /^(?:(?:오늘|어제|그제|내일|모레)\s*)?(?:(?:식단|물|수분)(?:\s|을|은|섭취|총|메모|완료|$)|(?:점심|저녁)\s+(?:식품\s*)?(?:단백질|밥))/.test(message.trim());
export function parseDietCommand(message: string): DietCommandChange {
  ...
  throw new Error('오늘 기록만 지원합니다. 예: ‘오늘 물 총 500ml 기록해줘’, ‘오늘 점심 단백질 30g 기록해줘’, ‘오늘 저녁 밥 80g 기록해줘’, ‘오늘 식단 메모 추가: 점심 닭가슴살’. 물은 오늘 총량, 단백질·밥량은 해당 끼니의 총량으로 말씀해 주세요.');
}
export function isDietCommandProposal(value: unknown): value is DietCommandProposal {
  const valid = object(value) && value.domain === 'diet' && typeof value.ownerId === 'string'
    ...
    && isDietChange(value.change) && isDietSnapshot(value.expected, value.change.kind === 'meal') && JSON.stringify(value.expected).length <= 200000
...
export function describeDietSnapshot(snapshot: DietDaySnapshot, change: DietCommandChange | 'water' | 'memo') {
...
export function dietCommandLabel(change: DietCommandChange) {
  return change.kind === 'meal' ? `${change.slot === 'lunch' ? '점심' : '저녁'} ${change.field === 'protein' ? '식품 단백질' : '밥량'}` : change.kind === 'water' ? '수분 총량' : '식단 메모 추가';
}
