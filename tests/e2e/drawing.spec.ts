...
test('drawing: first dot, ink undo, lost save reply, reload, private CAS and preserved older records', async ({page,qa}) => {
  ...
  await page.getByRole('link',{name:/그림 연습/}).click(); await showDrawingTools(page);
  ...
  await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 1장',exact:true}).click();await page.getByRole('button',{name:'열고 이어 그리기',exact:true}).click();
...
for (const authored of pack.lessons.slice(1,8)) {
  test(`drawing ${authored.id}: both examples, every action, paper reference and saved continuation`, async ({page,qa}) => {
    await login(page,qa.account); await synced(page); const before=await qa.read();
    await page.setViewportSize({width:390,height:844}); await page.goto('/growth/drawing'); await showDrawingTools(page);
    ...
    for (const [variant,example] of authored.examples.entries()) {
      ...
      await page.reload(); await showDrawingTools(page);
...
for (const authored of pack.lessons.slice(8,16)) {
  test(`drawing ${authored.id}: construction overlay, easier guides, both variants and saved part evidence`, async ({page,qa}) => {
    ...
    for(const [variant,ex] of authored.examples.entries()) {
      ...
      await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:`내 그림 ${variant+1}장`,exact:true}).click();
...
test('drawing D27: revise a copy of a saved drawing, restore the original reference and preserve the original',async({page,qa})=>{
  ...
  await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 2장',exact:true}).click();
...
test('drawing D33 D34: previous memory source remains untouched and reference snapshot survives deletion',async({page,qa})=>{
  await login(page,qa.account);await synced(page);const before=await qa.read();await page.goto('/growth/drawing'); await showDrawingTools(page);
  await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();
  await page.locator('#drawing-map summary').nth(3).click();
...
for (const authored of parsePack(pack).lessons.slice(34,42)) {
  test(`drawing ${authored.id}: one change preserves baseline, choices, ink, comparison and reload`,async({page,qa},testInfo)=>{
    ...
    await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();
    ...
    for(const [i,ex] of authored.examples.entries()) {
      ...
      await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:`내 그림 ${i+1}장`,exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').first().getByRole('button',{name:'열고 이어 그리기',exact:true}).click();
...
for (const authored of parsePack(pack).lessons.slice(42,52)) {
  test(`drawing ${authored.id}: analyze, assemble, hints, exact saved bodies and restored ink`,async({page,qa},testInfo)=>{
    ...
    await login(page,qa.account);await synced(page);const before=await qa.read();await page.setViewportSize({width:390,height:844});
    await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(5).click();await page.getByRole('button',{name:`${authored.id} · ${authored.title}`,exact:true}).click();
...
test('drawing D46: reuse owned D45 analysis, preserve saved source and restore both surfaces',async({page,qa})=>{
  await login(page,qa.account);await synced(page);const before=await qa.read();await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(5).click();await page.getByRole('button',{name:/^D45 ·/}).click();await page.getByRole('button',{name:'같은 목표의 다른 그림',exact:true}).click();
  ...
  await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 2장',exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').filter({hasText:'D46'}).getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await expect(practice.getByLabel('저장한 D45 분석',{exact:true})).toHaveValue(source.id);await expect(practice.getByRole('button',{name:'빈 공간에 다시 조립',exact:true})).toHaveAttribute('aria-pressed','true');await practice.getByRole('button',{name:'원본 위에서 나누기',exact:true}).click();await expect(practice.getByRole('button',{name:'되돌리기',exact:true})).toBeEnabled();expect(await qa.read()).toEqual(before);
});
...
for (const authored of parsePack(pack).lessons.slice(52,60)) {
  test(`drawing ${authored.id}: gesture directions, separate ink, every pose and restored checks`,async({page,qa},testInfo)=>{
    ...
    await login(page,qa.account);await synced(page);const before=await qa.read();await page.setViewportSize({width:390,height:844});
    await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(6).click();await page.getByRole('button',{name:`${authored.id} · ${authored.title}`,exact:true}).click();
...
for (const authored of parsePack(pack).lessons.slice(60,70)) {
 test(`drawing ${authored.id}: character features, variants, ink and restored comparison`,async({page,qa},testInfo)=>{
  test.setTimeout(180_000);await login(page,qa.account);await synced(page);const before=await qa.read();await page.setViewportSize({width:390,height:844});await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(7).click();await page.getByRole('button',{name:`${authored.id} · ${authored.title}`,exact:true}).click();
  ...
  for(const [i,ex] of authored.examples.entries()) {
   ...
   await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:`내 그림 ${i+1}장`,exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').first().getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await expect(practice.getByLabel('캐릭터 비교 메모',{exact:true})).toHaveValue(`${ex.id}: 귀 모양과 눈 간격`);await expect(practice.getByLabel('캐릭터 특징 비교 완료',{exact:true})).toBeChecked();await expect(practice.getByRole('button',{name:'되돌리기',exact:true})).toBeEnabled();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  }
  expect(await qa.read()).toEqual(before);
 });
}
...
for(const capstone of ['D65','D70'])test(`drawing ${capstone}: own collection copy preserves originals and restores snapshots`,async({page,qa})=>{
 ...
 const practice=page.getByRole('region',{name:'같은 캐릭터 연습'});
 const start=async(id:string)=>{await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(7).click();const lesson=pack.lessons.find(l=>l.id===id)!;await page.getByRole('button',{name:`${id} · ${lesson.title}`,exact:true}).click();await expect(practice).toBeVisible();};
...
for(const authored of parsePack(pack).lessons.slice(70,80))test(`drawing ${authored.id}: original creator variants, paper notes and exact restored state`,async({page,qa},testInfo)=>{
 test.setTimeout(180_000);await login(page,qa.account);await synced(page);const before=await qa.read();await page.setViewportSize({width:390,height:844});await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(8).click();await page.getByRole('button',{name:`${authored.id} · ${authored.title}`,exact:true}).click();
 ...
 for(const [i,ex] of authored.examples.entries()){
  ...
  await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:`내 그림 ${i+1}장`,exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').first().getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await expect(area.getByLabel('창작 비교 메모',{exact:true})).toHaveValue(saved.document.original.note);await expect(area.getByLabel('내 캐릭터 비교 완료',{exact:true})).toBeChecked();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
 }
 expect(await qa.read()).toEqual(before);
});
...
for(const motif of ['sprout','bird'])test(`drawing D80: ${motif} own D71 to D80 project preserves originals and exports actual card`,async({page,qa},testInfo)=>{
 ...
 for(let n=71;n<=80;n++){
  const id=`D${n}`,lesson=pack.lessons[n-1];await page.goto('/growth/drawing'); await showDrawingTools(page);await expect(page.getByRole('button',{name:'이어서 연습하기',exact:true})).toBeEnabled();await page.locator('#drawing-map summary').nth(8).click();await page.getByRole('button',{name:`${id} · ${lesson.title}`,exact:true}).click();if(motif==='bird')await page.getByRole('button',{name:'같은 목표의 다른 그림',exact:true}).click();
  ...
  else if(n===79){await draw();await page.getByRole('button',{name:'잠깐 쉬기 · 저장',exact:true}).click();await expect.poll(async()=>{const r=(await qa.account.client.from('growth_drawing_attempts').select('*')).data;return r?.some(a=>a.document.lesson.id===id&&a.status==='draft');}).toBe(true);await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 9장',exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').filter({hasText:id}).getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await area.getByRole('button',{name:'두 번째 자세 연습장',exact:true}).click();await draw();}
 ...
 await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 10장',exact:true}).click();await page.getByRole('region',{name:'내 그림 앨범'}).locator('article').filter({hasText:'D80'}).getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await expect(area.getByLabel('캐릭터 이름',{exact:true})).toHaveValue('새봄');await expect(area.getByRole('article',{name:'내 캐릭터 소개 카드'}).getByLabel('저장한 그림 미리보기')).toHaveCount(7);expect(await qa.read()).toEqual(before);
});
...
for (const project of pack.projects) {
  test(`drawing ${project.id}: four saved sessions, reload, own source preservation and project card`, async ({page,qa},testInfo) => {
    ...
    for(let i=0;i<4;i++) {
      ...
      await expect(practice.getByRole('status')).toContainText('저장했어요');
      const rows=(await qa.account.client.from('growth_drawing_attempts').select('*')).data!;
    ...
    await page.reload(); await showDrawingTools(page);await page.getByRole('button',{name:'내 그림 2장',exact:true}).click();await page.getByLabel('단계 필터').selectOption('10');await page.getByRole('button',{name:'열고 이어 그리기',exact:true}).click();await expect(practice.getByLabel('이번 회차 메모')).toHaveValue(saved.document.project.notes[3]);
