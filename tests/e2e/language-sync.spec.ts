...
test('language settings save immediately, preserve an edit during PATCH, and survive relogin', async ({ page, qa }) => {
  ...
  await page.getByRole('button', { name: /^7\s*개$/ }).click();
  ...
  await page.getByRole('button', { name: /^10\s*개$/ }).click();
  await page.getByRole('button', { name: '설정 저장', exact: true }).click();
  await expect(page.getByText('학습 기록 · 기기 저장, 서버 반영 대기', { exact: true })).toBeVisible();
  hold.release();

  await expect.poll(async () => goal(await qa.readLanguage()), { message: 'The newest language setting reached the actual database' }).toBe(10);
  await expect(page.getByText('학습 기록 · 서버 저장 확인', { exact: true })).toBeVisible();
  ...
  await expect(page.getByText('학습 기록 · 서버 저장 확인', { exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: /^10\s*개$/ })).toHaveAttribute('aria-pressed', 'true');
