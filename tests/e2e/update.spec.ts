...
test('production service worker waits during editing, then activates after save and explicit refresh', async ({ page, qa }) => {
  ...
  try {
    ...
    expect(navigations, 'The first worker installation did not reload/interfere with sign-in').toBe(1);
    ...
    await saveMeal(page, 'CI draft through worker update'); await mealSaved(page, qa, 'CI draft through worker update');
