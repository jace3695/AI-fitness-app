...
test('last-meal clock review cancels, recovers lost reply, persists fasting preview and undoes at 320px',async({page,qa})=>{
  ...
  await page.getByRole('link',{name:'식단 기록 보기 →',exact:true}).click();await expect(page.getByLabel('마지막 음식·프로틴 섭취시간')).toHaveValue('00:00');const fastingSection=page.locator('section').filter({has:page.getByRole('heading',{name:'공복 기록',exact:true})});await expect(fastingSection.getByText('당일 12:00',{exact:true})).toBeVisible();await expect(fastingSection.getByText('당일 14:00',{exact:true})).toBeVisible();await page.reload();await expect(page.getByLabel('마지막 음식·프로틴 섭취시간')).toHaveValue('00:00');await synced(page);
