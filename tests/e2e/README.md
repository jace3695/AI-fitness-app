...
P2 검사는 Chromium과 WebKit의 320px 식단 화면에서 사진을 고른 것만으로는 외부 전송하지 않고, 명시적 분석 뒤 추정값 확인을 거쳐 기존 식단 저장 경로로만 반영하는지 확인한다. 실제 API의 비로그인 차단·로그인 후 잘못된 본문 차단을 먼저 확인하고, 분석 응답만 합성해 인증 헤더·전송 시점·사진 비저장·실제 격리 DB 반영을 검증한다. 이 결과를 실제 AI 제공자의 판독 정확도 통과로 확대하지 않는다. 390px 성장 화면에서는 선호 요일과 주간 목표를 한 번에 저장하고 RLS로 다른 계정의 수정을 차단하는지 확인한다. AI 연이는 최근 7일의 합성 할 일·예산·운동·식단·언어·성장 데이터를 읽어 통합 브리핑을 만들되 외부 AI 요청을 보내지 않는지 검사한다.
