...
test('PIN status failure keeps content closed and explicit retry uses a real successful response', async ({ page, qa }, testInfo) => {
  let fail = true;
  ...
  await page.screenshot({ path: `.e2e/evidence/pin-status-error-${testInfo.project.name}.png` });
  fail = false;
...
test('delayed PIN status never exposes content before confirmation', async ({ page, qa }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: protectedHeading, exact: true })).toHaveCount(0);
    await page.screenshot({ path: `.e2e/evidence/pin-status-pending-${testInfo.project.name}.png` });
    release();
    await expect(page.getByRole('heading', { name: protectedHeading, exact: true })).toBeVisible();
    expect(await qa.read()).toEqual(original);
