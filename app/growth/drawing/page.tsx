    ...
    <div className="mx-auto max-w-5xl space-y-5 px-4 py-6">
      ...
      {owned && doc && <>
        ...
        <section className="drawing-card" aria-label="한 동작씩 보기">
          ...
          {doc.example.lines.some(l => l.group === "guide") && <div className="mb-3"><button className="drawing-button" disabled={!original} aria-pressed={referenceOverlay && original} onClick={() => setReferenceOverlay(!referenceOverlay)}>{referenceOverlay && original ? "완성 외곽 겹치기 끄기" : "완성 외곽 겹치기"}</button><p className="mt-2 text-xs">회색 외곽과 도형의 자리를 비교해요. 겹친 도형은 지우지 않아도 돼요. 원본을 숨기면 겹쳐보기도 숨겨져요.</p></div>}
