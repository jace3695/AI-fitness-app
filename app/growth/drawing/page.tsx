"use client";
import ProjectPractice from "@/components/drawing/ProjectPractice";
import {projectEligible,projectLesson,projectState} from "@/lib/drawing/projects";
import OriginalPractice from "@/components/drawing/OriginalPractice";
import {originalEligible} from "@/lib/drawing/original";
...
import PracticeOptions from "@/components/drawing/PracticeOptions";
...
export default function DrawingPage() {
  ...
  const activeId = owned?.id;
  useEffect(() => {
    if (activeId) workHeading.current?.scrollIntoView({ block: "start" });
  }, [activeId]);
  const next = recommend(pack, records.records);
  const prepared = pack.lessons.filter(playable);
  const doneIds = new Set(records.records.filter(a => a.status === "completed" && !a.document.lesson.projectPractice).map(a => a.document.lesson.id));
  function update(patch: Partial<DrawingDocument>) {
    if (!owned || records.busy) return;
    setAttempt({ ...owned, status: "draft", document: { ...owned.document, ...patch } }); setDirty(true); setReview(false); setLocalStatus("");
  }
  ...
  async function start(lesson: Lesson, exampleId?: string, help?: Help, short = false) {
    ...
    setDirty(true); setReview(false); setLocalStatus(""); setLibraryOpen(false); setEasy(short); setOriginal(lesson.stage !== 4); setOverlay(true); setReferenceOverlay(false); setAlbum(false); setCompare("");
  }
  ...
  const draft = records.records.find(a => a.status === "draft");
  const recovery = records.recovery[0];
  const suggested = next && pack.lessons.find(l => l.id === next.lessonId);
  const resumeLabel = recovery?.attempt.document.lesson.title ?? draft?.document.lesson.title;
  async function begin() {
    if (recovery) { if (await preserve()) { setAttempt(recovery.attempt); setDirty(recovery.pending); setOriginal(recovery.attempt.document.lesson.stage !== 4); setReview(false); setLocalStatus(""); setLibraryOpen(false); } }
    else if (draft) await open(draft);
    else if (next && suggested) await start(suggested, next.exampleId, next.help, next.short);
  }
  const actions = doc && !doc.lesson.projectPractice && <div className="drawing-step-actions">
    <button className="drawing-button" disabled={records.busy || doc.step === 0} onClick={() => update(doc.lesson.memoryPractice ? memoryStep(doc, doc.step - 1) : { step: doc.step - 1 })}>이전 행동</button>
    <button className="drawing-primary" disabled={records.busy || doc.step >= doc.lesson.steps.length - 1 || (!!doc.lesson.memoryPractice && doc.step === (doc.lesson.id === "D33" ? 1 : 0) && memoryState(doc).selected.length < 2)} onClick={() => { update(doc.lesson.memoryPractice ? memoryStep(doc, doc.step + 1) : { step: doc.step + 1 }); workHeading.current?.scrollIntoView({block:"start"}); }}>다음 행동</button>
    {!simple && !doc.lesson.memoryPractice && <button className="drawing-button" aria-pressed={original} onClick={() => setOriginal(!original)}>{original ? "원본 숨기기" : "원본 다시 보기"}</button>}
  </div>;

  return <main data-drawing-focus={simple && owned ? "true" : undefined} className="drawing-page min-h-screen bg-[#f6f4fb] pb-40 text-slate-800">
    ...
    <div className="mx-auto max-w-5xl space-y-5 px-4 py-6">
      <section className="drawing-card drawing-welcome" aria-label="그림 연습 시작">
        {!owned && <>
          <p className="text-sm font-semibold text-violet-700">하루 한 그림 · 천천히 해도 괜찮아요</p>
          <h1 className="mt-2 text-2xl font-bold">그림이 처음인가요?</h1>
          <p className="mt-2 text-slate-600">무엇을 그릴지 고민하지 마세요. 점 하나부터 같이 시작해요.</p>
          <ol className="drawing-three-steps"><li><b>1</b> 시작 버튼 누르기</li><li><b>2</b> 한 동작씩 따라 그리기</li><li><b>3</b> 저장하고 쉬기</li></ol>
          <div className="drawing-start-card">
            {!resumeLabel && suggested && <div className="drawing-start-picture"><Diagram example={suggested.examples.find(e=>e.id===next?.exampleId) ?? suggested.examples[0]} original /></div>}
            <div><p className="text-sm text-violet-700">{resumeLabel ? "하던 그림이 있어요" : "오늘 같이 그릴 그림"}</p><h2 className="mt-1 text-lg font-bold">{resumeLabel ?? (suggested ? suggested.title : "그림을 준비하고 있어요")}</h2>
              <p className="my-3 text-sm">{resumeLabel ? "마지막으로 하던 곳에서 계속할 수 있어요." : "손가락이나 마우스로 그릴 수 있어요. 잘 그리지 않아도 괜찮아요."}</p>
              <button className="drawing-primary drawing-start-button" disabled={!records.ready || !records.owner || (!next && !draft && !recovery)} onClick={()=>void begin()}>{!simple ? "이어서 연습하기" : resumeLabel ? "하던 그림 이어 그리기" : records.records.length ? "오늘의 그림 시작하기" : "처음 그림 그리기"}</button>
              {!records.ready && <p role="status" className="mt-2 text-sm">저장한 그림을 확인하고 있어요…</p>}
            </div>
          </div>
          {!simple && records.recovery.map(d => <button key={d.attempt.id} className="drawing-button mt-2" onClick={()=>{setAttempt(d.attempt);setDirty(d.pending);setOriginal(d.attempt.document.lesson.stage!==4);}}>{d.attempt.document.lesson.id} · 이 기기의 그림 복구하기</button>)}
        </>}
      ...
      {album && <section className="drawing-card" aria-label="내 그림 앨범"><h2 className="text-lg font-bold">내 그림</h2><label className="my-3 block text-sm">단계 필터 <select className="drawing-input" value={filter} onChange={e => setFilter(Number(e.target.value))}><option value={0}>모든 단계</option><option value={10}>지속 프로젝트</option>{pack.stages.map(s => <option key={s.id} value={s.id}>{s.id}. {s.title}</option>)}</select></label>
        {records.recovery.filter(d => d.attempt.id !== owned?.id).map(draft => <button key={draft.attempt.id} className="drawing-button mb-3 mr-2" onClick={async () => { if (await preserve()) { setAttempt(draft.attempt); setDirty(true); setAlbum(false); } }}>{draft.attempt.document.lesson.id} · 미전송 그림 복구</button>)}{!records.records.length && <p>저장한 그림이 여기에 모여요.</p>}<div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{records.records.filter(a => !filter || (filter===10 ? !!a.document.lesson.projectPractice : !a.document.lesson.projectPractice && a.document.lesson.stage === filter)).map(a => <article key={a.id} className="min-w-0 rounded-2xl border p-3"><DrawingCanvas key={a.id + a.revision} strokes={a.document.strokes.length ? a.document.strokes : a.document.structure?.analysis ?? a.document.gesture?.trace ?? []} preview />{a.document.photo && <Image src={a.document.photo} alt="종이 그림" width={200} height={200} unoptimized className="h-auto w-full" />}<h3 className="mt-2 text-sm font-bold">{a.document.lesson.id} {a.document.lesson.title}</h3><p className="my-2 text-xs">{new Date(a.updated_at).toLocaleDateString("ko-KR")} · {a.status === "draft" ? "진행 중" : "시도 완료"}</p><button className="drawing-button" onClick={() => void open(a)}>열고 이어 그리기</button><button className="drawing-button mt-2" disabled={records.busy} onClick={async () => { if (window.confirm("이 그림과 자기확인만 삭제할까요? 다른 기록은 유지돼요.") && await records.remove(a)) { if (a.id === owned?.id) { setAttempt(null); setDirty(false); } } }}>삭제</button></article>)}</div>
      </section>}
      {owned && doc && <>
        <section className="drawing-card" aria-label="현재 과제"><p className="text-sm font-semibold text-violet-700">{doc.lesson.projectPractice ? "내 그림으로 새롭게 그리기" : `${doc.lesson.stage}단계 연습`} · {doc.lesson.id}</p><h2 className="mt-1 text-xl font-bold">{doc.lesson.title}</h2><p className="mt-2">오늘 하나만: {doc.lesson.goal}</p>
          {simple && <div className="mt-4"><button className="drawing-button" disabled={records.busy || photoBusy} onClick={()=>void save(false)}>{records.busy ? "저장 중…" : "저장하고 쉬기"}</button><p role="status" className="mt-2 text-sm">{localStatus === "클라우드 저장 확인 완료" ? "저장했어요. 화면을 닫아도 나중에 이어 할 수 있어요." : dirty ? "아직 저장 전이에요. 쉬기 전에 저장 버튼을 눌러 주세요." : "저장한 그림을 열었어요. 천천히 이어 그려요."}</p></div>}
          <PracticeOptions simple={simple} title="종이에 그리기 · 시간 · 다른 예제">
          <fieldset disabled={records.busy || photoBusy} className="mt-4 flex flex-wrap gap-3"><label className="text-sm">그리는 곳<select className="drawing-input" value={doc.tool} onChange={e => update({ tool: e.target.value as DrawingDocument["tool"], ...(doc.lesson.projectPractice ? {project:{...projectState(doc),saved:[false,false,false,false],compared:false},step:0,check:"unconfirmed"} : {}) })}><option value="app">앱 안에서</option><option value="paper">종이·연필</option><option value="external">다른 그림 앱</option></select></label><label className="text-sm">연습 시간<select className="drawing-input" value={doc.short ? 5 : doc.minutes} onChange={e => update({ minutes: Number(e.target.value), short: Number(e.target.value) === 5 })}><option value={5}>5분 · 일부만 시도</option><option value={10}>10분</option><option value={20}>20분</option></select></label></fieldset>
          ...
          <div className="mt-4 flex flex-wrap gap-2"><button className="drawing-button" aria-pressed={easy} onClick={() => { setEasy(!easy); if (!easy) update({ short: true }); }}>더 쉽게 · 일부만</button>{!doc.lesson.projectPractice && <button className="drawing-button" onClick={() => { const lesson = pack.lessons.find(l => l.id === doc.lesson.id) ?? doc.lesson; void start(lesson, lesson.examples[(lesson.examples.findIndex(e => e.id === doc.example.id) + 1) % lesson.examples.length]?.id, doc.help); }}>같은 목표의 다른 그림</button>}<button className="drawing-button" onClick={() => void save(false)} disabled={records.busy}>잠깐 쉬기 · 저장</button></div>
        ...
        <section className="drawing-card" aria-label="한 동작씩 보기">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-violet-700">{doc.step + 1} / {doc.lesson.steps.length} · 지금 할 행동</p><h3 ref={workHeading} tabIndex={-1} className="drawing-work-heading mt-2 text-lg font-bold">{simple && doc.lesson.projectPractice ? ["1. 내 그림 고르기", "2. 첫 모습 그리기", "3. 다른 모습 더하기", "4. 비교하고 마무리"][doc.step] : step?.text}</h3></div></div>
          {!simple && actions}
          {simple && !doc.lesson.projectPractice && <p className="drawing-tip">설명을 보고 아래에 그려요. 다 했으면 ‘다음 행동’을 눌러요. 마지막에는 저장해요.</p>}
          {doc.lesson.projectPractice ? <ProjectPractice simple={simple} onStartBasic={()=>void start(pack.lessons.find(l=>l.id==="D01")!)} key={owned.id} attempt={owned} records={records.records} disabled={records.busy || photoBusy} onChange={update} onSave={patch=>save(false,patch)} /> : doc.lesson.originalPractice ? <OriginalPractice key={owned.id} attempt={owned} records={records.records} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.identityPractice ? <IdentityPractice key={owned.id} attempt={owned} records={records.records} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.gesturePractice ? <GesturePractice key={owned.id} attempt={owned} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.structurePractice ? <StructurePractice key={owned.id} attempt={owned} records={records.records} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.variationPractice ? <VariationPractice key={owned.id} attempt={owned} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.memoryPractice ? <MemoryPractice key={owned.id} attempt={owned} records={records.records} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : doc.lesson.practice?.mode === "copy" ? <CopyPractice key={owned.id} attempt={owned} records={records.records} original={original} easy={easy || doc.short} disabled={records.busy} onChange={update} /> : <>
          {!simple && <>
          <PracticeOptions simple={simple} title="완성 그림 · 인쇄용 밑그림">
          ...
          <p className="mb-3 text-xs text-slate-600">주황 점에서 시작해 보라 선을 따라 그려요. 화살표 쪽으로 움직이면 돼요.</p>
          {doc.tool === "app" ? <>
            {!simple && <>
            <PracticeOptions simple={simple} title="밑그림 도움 바꾸기"><label className="mb-3 block text-sm">도움 정도<select className="drawing-input" value={doc.help} onChange={e => { const help = Number(e.target.value) as Help; update({ help, usedHelp: Math.max(help, doc.usedHelp) as Help }); }}>{HELP_LABELS.map((label, i) => <option key={i} value={i}>{label}</option>)}</select></label>
            <button className="drawing-button mb-3" aria-pressed={overlay} onClick={() => setOverlay(!overlay)}>{overlay ? "밑그림 숨기기" : "밑그림 다시 보기"}</button>
            </PracticeOptions>
            </>}
            <DrawingCanvas simple={simple} key={owned.id} strokes={doc.strokes} onChange={strokes => update({ strokes })} disabled={records.busy} guide={overlay && <Diagram example={doc.example} lesson={doc.lesson} step={doc.step} help={doc.help} referenceOverlay={referenceOverlay && original} easy={easy || doc.short} />} />
          ...
          <PracticeOptions simple={simple} title="그림의 부위 찾아보기">{original && <PartFinder key={owned.id} example={doc.example} checked={doc.partChecks ?? []} disabled={records.busy} onCheck={partChecks => update({partChecks})} onHelp={() => update({usedHelp: Math.max(1, doc.usedHelp) as Help})} />}</PracticeOptions>
          </>}
          {simple && actions}
          {simple && !doc.lesson.projectPractice && <button className="drawing-button" disabled={records.busy || photoBusy} onClick={()=>void save(false)}>여기까지 저장</button>}
          {simple && !doc.lesson.projectPractice && doc.step === doc.lesson.steps.length-1 && <p className="drawing-tip">마지막 동작이에요! 아래 ‘짧게 돌아보기’에서 오늘 연습을 저장해요.</p>}
          <PracticeOptions simple={simple} title="종이 그림 사진 가져오기">
        ...
        <section className="drawing-card" aria-label="오늘의 자기확인"><PracticeOptions simple={simple && !review && (doc.lesson.projectPractice ? !projectEligible(doc) : doc.step < doc.lesson.steps.length-1)} title="오늘 연습 마치기"><h3 className="text-lg font-bold">짧게 돌아보기</h3><p className="mt-3">{doc.lesson.check}</p><fieldset disabled={records.busy} className="mt-3 flex flex-wrap gap-2">{([["independent", "스스로 해봤어요"], ["assisted", "도움을 받았어요"], ["difficult", "아직 어려워요"], ["unconfirmed", "아직 확인 전"]] as [Check, string][]).map(([value, label]) => <button className={doc.check === value ? "drawing-primary" : "drawing-button"} key={value} disabled={value === "independent" && (!!doc.memory?.copyMode || !variationEligible(doc) || !structureEligible(doc) || !gestureEligible(doc) || !identityEligible(doc) || !originalEligible(doc) || !projectEligible(doc))} aria-pressed={doc.check === value} onClick={() => update({ check: value })}>{label}</button>)}</fieldset>
          ...
          </PracticeOptions><p className="my-3 text-xs text-slate-500">자기확인은 그림을 AI가 채점한 결과가 아니에요. 그림 분석 없이 다음 과제를 추천해요.</p>
          <div className="flex flex-wrap gap-2"><button className="drawing-primary" disabled={records.busy || photoBusy || !projectEligible(doc)} onClick={() => void save(true)}>{records.busy ? "저장 확인 중…" : simple ? "오늘 연습 마치고 저장" : "시도 마치고 저장"}</button><button className="drawing-button" disabled={records.busy || photoBusy} onClick={() => void save(false)}>진행 중 저장</button></div><PracticeOptions simple={simple} title="그림 파일 내려받기"><div className="flex flex-wrap gap-2"><button className="drawing-button" onClick={() => exportAttempt(owned)}>편집 가능한 기록 내보내기</button><button disabled={!!doc.lesson.projectPractice || (!!doc.lesson.originalPractice && doc.lesson.id === "D80")} className="drawing-button" onClick={() => { const canvas = document.createElement("canvas"); canvas.width = canvas.height = 1200;
            ...
            } else paintStrokes(canvas, doc.strokes, true); canvas.toBlob(blob => { if (blob) download(blob, `${doc.lesson.id}-my-drawing.png`); }, "image/png"); }}>내 그림 PNG</button></div></PracticeOptions>
          <p role="status" className="mt-3 text-sm">{localStatus}</p></PracticeOptions>
        </section>
        {review && <section className="drawing-card" aria-label="연이의 연습 정리"><h3 className="text-lg font-bold">오늘 연습 정리</h3><dl className="mt-3 space-y-4">{feedback(owned, pack, records.records).map(item => <div key={item.title}><dt className="font-semibold">{item.title}</dt><dd className="mt-1 text-sm text-slate-600">{item.body}</dd></div>)}</dl>{next && !doc.lesson.projectPractice && <button className="drawing-primary mt-5" onClick={() => void start(pack.lessons.find(l => l.id === next.lessonId)!, next.exampleId, next.help, next.short)}>추천 과제 시작</button>}</section>}
      </>}
      <button className="drawing-button" aria-expanded={resourcesOpen || !simple} onClick={()=>setResourcesOpen(!resourcesOpen)}>참고 자료 · 템플릿 보기</button>
      {(!simple || resourcesOpen) && <>
      ...
      {(!simple || libraryOpen) && <div ref={libraryRef} id="drawing-library" className="space-y-5">
      <section id="drawing-map" className="drawing-card"><h2 className="text-lg font-bold">다른 수업 고르기</h2><p className="my-3 text-sm text-slate-600">처음이라면 1단계부터 해 보세요. 익숙해지면 다른 수업도 골라 볼 수 있어요. 준비된 수업 {prepared.length}개.</p>{pack.stages.map(stage => <details key={stage.id} className="border-t py-4"><summary className="cursor-pointer font-semibold">{stage.id}. {stage.title} {confirmedStage(pack, records.records, stage.id) ? "· 자기확인됨" : "· 목표 확인 전"}</summary><p className="my-3 text-sm">단계 목표: {stage.criterion}</p><ul className="space-y-2">{pack.lessons.filter(l => l.stage === stage.id).map(lesson => <li key={lesson.id}><button className="drawing-button w-full text-left" disabled={!playable(lesson) || !records.ready || !records.owner || records.busy} onClick={() => void start(lesson)}>{lesson.id} · {lesson.title} {!playable(lesson) && "· 시각 자료 준비 중"}</button></li>)}</ul><p className="mt-3 text-xs text-slate-500">준비된 수업은 다음 단계도 맛볼 수 있어요. 맛보기를 이전 단계의 숙련으로 기록하지 않아요.</p></details>)}</section>
      <section className="drawing-card"><h2 className="text-lg font-bold">내 그림으로 새롭게 그리기</h2><p className="my-3 text-sm">저장한 내 그림을 골라 표정과 모습을 바꿔요. 그림이 처음이라면 기본 수업부터 해도 좋아요.</p>{pack.projects.map(project => <details key={project.id} className="border-b py-3"><summary className="cursor-pointer font-semibold">{project.id} · {project.title}</summary><ol className="my-3 list-inside list-decimal space-y-2 text-sm">{project.sessions.map(session => <li key={session}>{session}</li>)}</ol><p className="text-sm">확인할 점: {project.check}</p><div className="mt-3 flex flex-wrap gap-2"><button className="drawing-primary" disabled={!records.ready || !records.owner || records.busy} onClick={()=>{const draft=records.records.find(a=>a.document.lesson.id===project.id && a.status==="draft"); if(draft) void open(draft); else void start(projectLesson(pack,project.id));}}>{project.id} 시작·이어하기</button><button className="drawing-button" disabled={!records.ready || !records.owner || records.busy} onClick={()=>void start(projectLesson(pack,project.id))}>{project.id} 새 프로젝트</button></div></details>)}</section>
      </div>}
      {!simple && <section className="drawing-card text-sm"><h2 className="font-semibold">선택 이미지 피드백</h2><p className="mt-2 text-slate-600">그림 분석은 아직 연결 검증 중이에요. 기본 수업·자기확인·추천에는 AI 호출이 필요 없어요. 사진을 고르거나 그림을 그리는 것만으로 AI에 보내지 않아요.</p></section>}
    </div>
  </main>;
}
