    ...
    <div className="mx-auto max-w-5xl space-y-5 px-4 py-6">
      ...
      {owned && doc && <>
        <section className="drawing-card" aria-label="현재 과제"><p className="text-sm font-semibold text-violet-700">{doc.lesson.stage}단계 · {doc.lesson.id}</p><h2 className="mt-1 text-xl font-bold">{doc.lesson.title}</h2><p className="mt-2">오늘 하나만: {doc.lesson.goal}</p>
          ...
          {doc.lesson.id === "D42" && <label className="mt-4 block text-sm">익숙한 캐릭터 선택<select aria-label="익숙한 캐릭터 선택" className="drawing-input mt-1 w-full" disabled={records.busy} value={doc.example.id} onChange={e => { void start(doc.lesson, e.target.value); }}>{doc.lesson.examples.map(ex => <option key={ex.id} value={ex.id}>{ex.name}</option>)}</select></label>}
