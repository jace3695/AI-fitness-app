...
import { drawingLessonDayFromMetrics, getDrawingRoutineId } from "../data/drawingPractice";
import { DRAWING_ROUTINE_TITLE, GROWTH_CATEGORIES, GROWTH_ROUTINE_LIMIT, growthCategoryLabel, type GrowthCategoryId } from "../data/growthRoutines";
...
export default function GrowthPage() {
  ...
  const [drawingRoutineId, setDrawingRoutineId] = useState<string | null>(null);
  ...
  useEffect(() => {
    let active = true;
    if (!growthUserId) {
      setDrawingRoutineId(null);
      return () => { active = false; };
    }
    void getDrawingRoutineId(growthUserId).then((id) => {
      if (active) setDrawingRoutineId(id);
    });
    return () => { active = false; };
  }, [growthUserId]);

  const routineIdentityLoading = Boolean(growthUserId && !drawingRoutineId);
  const enabledRoutines = growth.routines.filter((routine) => routine.enabled);
  const manualRoutines = routineIdentityLoading ? [] : enabledRoutines.filter((routine) => routine.id !== drawingRoutineId);
  ...
  const addRoutine = async (event: FormEvent) => {
    ...
    if (value === DRAWING_ROUTINE_TITLE) {
      growth.setNotice("이 이름은 그림 전용 과정에서 사용합니다. 다른 이름을 입력해 주세요.");
      return;
    }
    setSaving(true);
  ...
  const importLegacyBackup = async () => {
    ...
    const result = await growth.importLegacyBackup();
    setLegacyImporting(false);
    if (result.error) growth.setNotice(result.error.message);
  };
  ...
  return (
    <main className="min-h-dvh bg-[#F5F4FA] pb-10 text-[#242231]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        <nav aria-label="자기계발 연습과 자료" className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <Link href="/growth/drawing" className="rounded-3xl bg-violet-50 p-4 text-center ring-1 ring-violet-100"><span className="text-2xl" aria-hidden="true">✏️</span><strong className="mt-2 block text-sm text-violet-900">그림 기초 연습</strong><span className="mt-1 block text-xs text-violet-700">왕초보 28회 과정</span></Link>
        ...
        <section className="mt-5 rounded-[28px] bg-white p-4 shadow-sm sm:p-6">
          <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-violet-600">클라우드 동기화</p><h2 className="mt-1 text-xl font-bold">나의 루틴</h2></div><button type="button" disabled={routineIdentityLoading} aria-expanded={editing} onClick={() => setEditing((value) => !value)} className="min-h-11 rounded-full bg-gray-100 px-4 text-xs font-bold text-gray-700 disabled:opacity-50">{editing ? "편집 닫기" : "루틴 편집"}</button></div>
          <p className="mt-2 text-xs leading-5 text-gray-500">운동과 일본어는 각 앱의 오늘 기록이 자동으로 반영됩니다.</p>
          {growth.legacyBackupAvailable ? <div className="mt-4 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-950 ring-1 ring-amber-200"><strong>이전 기기 루틴 백업이 있습니다</strong><p className="mt-1">공용 기기의 다른 계정 기록과 섞이지 않도록 자동으로 가져오지 않았습니다. 이 백업이 본인 것일 때만 현재 계정으로 가져오세요.</p><button type="button" disabled={legacyImporting || growth.loading} onClick={() => void importLegacyBackup()} className="mt-3 min-h-11 rounded-xl bg-amber-800 px-4 text-xs font-bold text-white disabled:opacity-40">{legacyImporting ? "안전하게 가져오는 중…" : "이 계정으로 가져오기"}</button></div> : null}
          ...
          <div className="mt-3 space-y-3">{growth.loading || routineIdentityLoading ? <p className="py-8 text-center text-sm text-gray-400">개인 루틴을 안전하게 동기화하고 있어요…</p> : enabledRoutines.map((routine) => {
            const completed = todayCompletedIds.has(routine.id);
            const isDrawing = routine.id === drawingRoutineId;
            return <article key={routine.id} className={`grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border p-4 ${completed ? "border-emerald-100 bg-emerald-50" : "border-gray-100"}`}>
              {isDrawing
                ? <span aria-label={completed ? "오늘 그림 수업 완료" : "그림 수업 미완료"} className={`grid h-11 w-11 place-items-center rounded-full border-2 text-lg font-bold ${completed ? "border-emerald-600 bg-emerald-600 text-white" : "border-violet-200 bg-violet-50 text-violet-600"}`}>{completed ? "✓" : "✎"}</span>
                : <button disabled={saving} type="button" onClick={() => void quickToggle(routine.id)} aria-label={`${routine.title} ${completed ? "완료 취소" : "빠른 완료"}`} className={`grid h-11 w-11 place-items-center rounded-full border-2 text-lg font-bold ${completed ? "border-emerald-600 bg-emerald-600 text-white" : "border-gray-300 text-transparent"}`}>✓</button>}
              <div className="min-w-0"><p className="text-xs font-bold text-violet-600">{isDrawing ? "그림 기초" : growthCategoryLabel(routine.category)}</p><h3 className={`mt-1 truncate font-bold ${completed ? "text-gray-500" : "text-gray-900"}`}>{routine.title}</h3><p className="mt-1 text-xs text-gray-500">목표 {routine.target_minutes}분{isDrawing ? " · 전용 화면에서 기록" : ""}</p></div>
              {isDrawing
                ? <Link href="/growth/drawing" className="inline-flex min-h-11 items-center rounded-xl bg-violet-600 px-4 text-xs font-bold text-white">{completed ? "기록 보기" : "수업 열기"}</Link>
                : <button type="button" disabled={Boolean(activeRoutine) || completed} onClick={() => startRoutine(routine.id)} className="min-h-11 rounded-xl bg-violet-600 px-4 text-xs font-bold text-white disabled:bg-gray-200 disabled:text-gray-500">{completed ? "완료" : activeRoutineId === routine.id ? "진행 중" : "시작"}</button>}
            </article>;
          })}</div>
          {growth.notice && <p role="status" className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-800">{growth.notice}</p>}
          {editing && !routineIdentityLoading && <div className="mt-5 rounded-3xl bg-[#F7F6FF] p-4 sm:p-5"><h3 className="font-bold">개인 루틴 편집</h3><p className="mt-1 text-xs leading-5 text-gray-500">운동·일본어는 위에서 자동 연결되며, 아래에서는 개인 루틴만 편집합니다. 변경 내용은 계정에 저장되어 iPhone과 iPad에서 함께 보입니다.</p><form onSubmit={addRoutine} className="mt-4 grid gap-2 sm:grid-cols-[auto_1fr_auto_auto]"><select value={category} onChange={(event) => setCategory(event.target.value as GrowthCategoryId)} className="min-h-12 rounded-xl bg-white px-3 text-sm ring-1 ring-gray-200">{GROWTH_CATEGORIES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={60} placeholder="예: 타자 정확도 연습" className="min-h-12 rounded-xl bg-white px-4 text-sm ring-1 ring-gray-200" /><label className="flex min-h-12 items-center gap-2 rounded-xl bg-white px-3 text-sm ring-1 ring-gray-200">목표 <input type="number" min={5} max={240} step={5} value={targetMinutes} onChange={(event) => setTargetMinutes(Number(event.target.value))} className="w-14 bg-transparent font-bold outline-none" />분</label><button disabled={saving || !title.trim() || growth.routines.length >= GROWTH_ROUTINE_LIMIT} className="min-h-12 rounded-xl bg-violet-600 px-5 text-sm font-bold text-white disabled:bg-gray-300">추가</button></form><div className="mt-4 space-y-2">{growth.routines.map((routine) => { const isDrawing = routine.id === drawingRoutineId; return <div key={routine.id} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-3"><button type="button" onClick={() => void growth.updateRoutine(routine.id, { enabled: !routine.enabled })} className={`min-h-10 rounded-lg px-3 text-xs font-bold ${routine.enabled ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{routine.enabled ? "사용 중" : "숨김"}</button><span className="min-w-0 flex-1 truncate text-sm font-semibold">{routine.title} · {routine.target_minutes}분</span>{isDrawing ? <span className="rounded-lg bg-violet-50 px-3 py-2 text-xs font-bold text-violet-600">전용 과정</span> : <button type="button" onClick={() => void growth.removeRoutine(routine.id)} className="min-h-10 rounded-lg bg-red-50 px-3 text-xs font-bold text-red-600">삭제</button>}</div>; })}</div></div>}
        </section>

        <section className="mt-5 rounded-[28px] bg-white p-4 shadow-sm sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-violet-600">실행 이력</p><h2 className="mt-1 text-xl font-bold">최근 기록</h2></div><button type="button" onClick={() => void growth.refresh()} className="rounded-full bg-gray-100 px-3 py-2 text-xs font-bold text-gray-600">새로고침</button></div><div className="mt-4 space-y-2">{recentSessions.length ? recentSessions.map((session) => { const routine = growth.routines.find((item) => item.id === session.routine_id); const statusLabel = session.status === "completed" ? "완료" : session.status === "partial" ? "진행" : "중단"; const drawingDay = drawingLessonDayFromMetrics(session.metrics); return <article key={session.id} className="grid grid-cols-[1fr_auto] gap-3 rounded-2xl bg-gray-50 p-4"><div><div className="flex flex-wrap items-center gap-2"><strong>{routine?.title ?? "삭제된 루틴"}</strong><span className={`rounded-full px-2 py-1 text-[11px] font-bold ${session.status === "completed" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{statusLabel}</span></div><p className="mt-1 text-xs text-gray-500">{session.session_date} · {session.actual_minutes}분{session.memo ? ` · ${session.memo}` : ""}</p></div>{drawingDay ? <Link href="/growth/drawing" className="inline-flex min-h-10 items-center rounded-lg bg-violet-50 px-3 text-xs font-bold text-violet-600">{drawingDay}회 보기</Link> : <button type="button" aria-label="기록 삭제" onClick={() => void growth.deleteSession(session.id)} className="text-xl text-gray-300">×</button>}</article>; }) : <p className="py-7 text-center text-sm text-gray-400">아직 실행 기록이 없습니다.</p>}</div></section>
