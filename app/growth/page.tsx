  ...
  return (
    <main className="min-h-dvh bg-[#F5F4FA] pb-10 text-[#242231]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        <section className="mt-5 rounded-[28px] bg-white p-4 shadow-sm sm:p-6"><p className="text-xs font-bold text-violet-600">연결된 앱</p><h2 className="mt-1 text-xl font-bold">오늘 전체 흐름</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><Link href={language.nextHref} className="rounded-3xl bg-blue-50 p-5 ring-1 ring-blue-100"><p className="text-xs font-bold text-blue-700">일본어</p><h3 className="mt-2 text-lg font-bold text-blue-950">{linkedLoading ? "불러오는 중" : `${language.completed}/${language.total} 완료`}</h3><p className="mt-1 text-sm text-gray-600">{language.nextLabel}</p></Link><Link href="/fitness" className="rounded-3xl bg-orange-50 p-5 ring-1 ring-orange-100"><p className="text-xs font-bold text-orange-700">운동</p><h3 className="mt-2 text-lg font-bold text-orange-950">{linkedLoading ? "불러오는 중" : fitness.title}</h3><p className="mt-1 text-sm text-gray-600">{fitness.detail}</p></Link></div></section>
      </div>
    </main>
  );
}
