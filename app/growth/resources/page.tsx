...
export default function GrowthResourcesPage() {
  ...
  return <main className="min-h-dvh bg-yeoni-bg pb-10 text-[#242231]">
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-9">
      ...
      <section className="rounded-[28px] bg-white p-5 shadow-sm sm:p-6"><p className="text-xs font-bold text-emerald-600">비공개 자료함</p><h1 className="mt-1 text-2xl font-bold">자료 추가</h1><p className="mt-2 text-sm text-gray-500">PDF·텍스트·Word·JPG·PNG·WebP, 파일당 최대 10MB</p><form onSubmit={upload} className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2"><input id="growth-resource-file" type="file" required disabled={saving} accept=".pdf,.txt,.md,.docx,.jpg,.jpeg,.png,.webp" onChange={chooseFile} className="min-h-12 min-w-0 w-full rounded-xl bg-gray-50 p-3 text-sm ring-1 ring-gray-200" /><input required value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} placeholder="자료 이름" className="min-h-12 min-w-0 w-full rounded-xl bg-gray-50 px-4 text-sm ring-1 ring-gray-200" /><select value={routineId} onChange={(event) => setRoutineId(event.target.value)} className="min-h-12 min-w-0 w-full rounded-xl bg-gray-50 px-3 text-sm ring-1 ring-gray-200"><option value="">루틴 연결 없음</option>{availableRoutines.map((routine) => <option key={routine.id} value={routine.id}>{routine.title}</option>)}</select><select value={classification} onChange={(event) => setClassification(event.target.value as GrowthResourceClassification)} className="min-h-12 min-w-0 w-full rounded-xl bg-gray-50 px-3 text-sm ring-1 ring-gray-200">{CLASSIFICATIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><textarea value={notes} onChange={(event) => setNotes(event.target.value)} maxLength={500} placeholder="메모(선택)" className="min-h-20 min-w-0 w-full rounded-xl bg-gray-50 p-3 text-sm ring-1 ring-gray-200 sm:col-span-2" /><button disabled={saving || !file || !title.trim()} className="min-h-12 min-w-0 w-full rounded-xl bg-emerald-600 text-sm font-bold text-white disabled:bg-gray-300 sm:col-span-2">{saving ? "저장 중…" : "비공개 자료 저장"}</button></form>{growth.notice && <p role="status" className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-800">{growth.notice}</p>}</section>
