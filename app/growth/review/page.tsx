...
export default function GrowthReviewPage() {
  ...
  const saveDecision = async (decision: "applied" | "partial" | "kept") => {
    ...
    if (decision !== "kept") {
      const chosen = latest.suggestions.filter((suggestion) => selected.includes(suggestion.id)
        && suggestion.routineId
        && suggestion.recommendedMinutes
        && growth.routines.some((routine) => routine.id === suggestion.routineId && !isRetiredGrowthRoutine(routine)));
