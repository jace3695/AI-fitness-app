...
export default function GrowthReviewPage() {
  ...
  const saveDecision = async (decision: "applied" | "partial" | "kept") => {
    if (!supabase || !growth.user || !latest || latest.decision || decidingRef.current || generating) return;
    ...
    try {
      ...
      await growth.refresh();
      await load();
      growth.setNotice(error && typeof error === 'object' && 'message' in error ? String(error.message) : '저장 결과를 확인하지 못했어요. 최신 코칭을 확인한 뒤 다시 시도해 주세요.');
