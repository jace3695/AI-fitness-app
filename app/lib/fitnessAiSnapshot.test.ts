...
test("AI 운동 스냅샷은 최근 기록·운동 방식·세트 상세와 통증 정보를 정규화한다", () => {
  ...
  stores.workouts["2026-08-27"] = { workoutDone: true, workoutStatus: "completed", workoutFatigue: 4, workoutPain: true, workoutBackStatus: "worse", workoutNeurologicalSymptoms: ["tingling"], workoutPainExercise: "고블릿 스쿼트", workoutPainSet: 1, workoutMethod: { method: "circuit", rounds: 2, restSeconds: 60, workSeconds: 30 }, workoutExerciseRecords: [{ exerciseName: "고블릿 스쿼트", status: "completed", painScore: 3, sets: [{ setNumber: 1, completed: true, reps: 10, weightKg: 7, plannedReps: 8, restAfterSeconds: 45 }] }] };
  ...
  assert.equal(snapshot.recentSessions[0].backStatus, "worse");
  assert.deepEqual(snapshot.recentSessions[0].neurologicalSymptoms, ["tingling"]);
  ...
  assert.equal(snapshot.profile.currentWeightKg, 86);
  assert.equal(snapshot.profile.currentWeightSource, "user-provided-baseline");
  assert.match(snapshot.currentProgram.structure, /월·수·금/);
});

test("AI 운동 스냅샷은 미래·오래된 기록을 빼고 최근 28일만 전달한다", () => {
  const stores = emptyStores();
  for (let day = 1; day <= 31; day += 1) stores.workouts[`2026-07-${String(day).padStart(2, "0")}`] = true;
  stores.workouts['2026-08-01'] = true;
  const snapshot = buildFitnessAiSnapshot(stores, new Date('2026-07-31T12:00:00'));
  assert.equal(snapshot.recentSessions.length, 28);
  assert.equal(snapshot.recentSessions[0].date, '2026-07-31');
  assert.equal(snapshot.recentSessions.at(-1)?.date, '2026-07-04');
});
