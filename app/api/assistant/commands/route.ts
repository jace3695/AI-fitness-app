...
const fields = 'id,operation,item_id,before_record,after_record,created_at,undone_at';
...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase.from('assistant_task_command_history').select(fields)
...
export async function POST(request: NextRequest) {
  ...
  if (body.decision === 'apply' && (!proposal || !['create', 'update'].includes(proposal.operation)
