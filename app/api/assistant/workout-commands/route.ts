...
const fields = 'user_id,id,record_date,before_values,after_values,created_at,undone_at';
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await db.from('assistant_workout_command_history').select(fields).eq('user_id', user.id)
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = body.decision === 'undo'
    ...
    : await db.rpc('apply_assistant_workout_command', {
      p_request_id: p.requestId, p_day: p.date, p_expected: p.expected,
      p_reset_markers: p.resetMarkers, p_expires_at: p.expiresAt,
    });
  if (error) return reply({ error: error.code === 'P0001' ? error.message : '운동 저장 결과를 확인하지 못했습니다. 같은 요청으로 다시 확인하거나 실행 이력을 확인해 주세요.' }, error.code === 'PGRST202' ? 503 : 409);
