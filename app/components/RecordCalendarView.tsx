...
export default function RecordCalendarView() {
  ...
  const selectedDiet = stores.diet[selected]; const selectedWater = stores.water[selected] || 0; const selectedDinner = stores.dinner[selected]; const selectedDinnerCarb = formatDinnerCarbRecord(stores.dinnerCarbs[selected]); const selectedLunchCarb = formatLunchCarbRecord(stores.lunchCarbs[selected]); const selectedLunchProtein = formatLunchProteinRecord(stores.lunchProteins[selected]);
  const anyRecord = Boolean(stores.workouts[selected] || selectedDiet || selectedWater || selectedDinner || stores.weights[selected] || stores.inbody[selected] || stores.notes[selected] || stores.dinnerCarbs[selected] || stores.lunchCarbs[selected] || stores.lunchProteins[selected]);
