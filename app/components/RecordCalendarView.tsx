...
import { DIET_GOAL_CHECK_ITEMS, FASTING_MODE_KEY, formatDinnerCarbRecord, formatLunchCarbRecord, formatLunchProteinRecord, getLocalDateKey } from '../data/dietPlans';
...
export default function RecordCalendarView() {
  ...
  const selectedDiet = stores.diet[selected]; const selectedRecovery = stores.recovery[selected]; const selectedWater = stores.water[selected] || 0; const selectedDinner = stores.dinner[selected]; const selectedDinnerCarb = formatDinnerCarbRecord(stores.dinnerCarbs[selected]); const selectedLunchCarb = formatLunchCarbRecord(stores.lunchCarbs[selected]); const selectedLunchProtein = formatLunchProteinRecord(stores.lunchProteins[selected]);
  const fastingModes = (typeof window === 'undefined' ? {} : JSON.parse(window.localStorage.getItem(FASTING_MODE_KEY) || '{}')) as Record<string, string>;
  ...
  return <div className="space-y-4">
    <section className="rounded-2xl bg-white border border-gray-100 p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-7 gap-1 text-center">{weekDays.map((day) => <div key={day} className="py-1 text-[11px] font-bold text-gray-400">{day}</div>)}{cells.map((key, idx) => {
        ...
        const badges = [stores.recovery[key]?.completedAsRecovery ? '회' : stores.workouts[key] ? '운' : '', stores.recovery[key]?.recoveryPriorityOnly ? '휴' : '', dietSuccess ? '식' : '', (stores.water[key] || 0) >= 2000 ? '💧' : '', stores.weights[key] ? 'W' : '', stores.inbody[key] ? 'I' : '', stores.notes[key] ? '✎' : '', hasSafetyAlert(stores.diet[key]) ? '⚠' : ''].filter(Boolean);
