...
import { DIET_GOAL_CHECK_ITEMS, getLocalDateKey } from '../data/dietPlans';
...
export default function RecordCalendarView() {
  ...
  const selectedDiet = stores.diet[selected]; const selectedWater = stores.water[selected] || 0; const selectedDinner = stores.dinner[selected];
  const anyRecord = Boolean(stores.workouts[selected] || selectedDiet || selectedWater || selectedDinner || stores.weights[selected] || stores.inbody[selected] || stores.notes[selected]);
