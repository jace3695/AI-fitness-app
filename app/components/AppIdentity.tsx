...
const APPS = {
  ...
  budget: { label: "가계부", Icon: WalletCards, tone: "bg-orange-100 text-orange-700", image: "/app-icons/budget.ico" },
...
export function AppIcon({ kind, className = "h-10 w-10" }: { kind: AppKind; className?: string }) {
  ...
  const image = kind === "assistant" ? "/app-icons/assistant.png" : kind === "budget" ? "/app-icons/budget.ico" : kind === "fitness" ? "/app-icons/fitness.png" : kind === "language" ? "/app-icons/language.png" : null;
