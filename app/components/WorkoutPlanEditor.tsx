...
export default function WorkoutPlanEditor({ settings, defaultGroups, onChange }: {
  ...
  return <section className="mb-4 rounded-3xl border border-[#D9D6FF] bg-white p-5 shadow-sm sm:p-6">
    ...
    {group.type === "choice" ? <p className="mt-4 rounded-xl bg-blue-50 p-3 text-sm text-blue-800">선택 유산소의 시간은 운동 화면에서 직접 입력할 수 있습니다.</p> : <div className="mt-5 space-y-3">
      ...
      {exercises.map((exercise) => {
        ...
        const ai: ExerciseTarget = { sets: numberFrom(exercise.sets, "세트"), reps: numberFrom(exercise.sets, "회"), durationMinutes: numberFrom(exercise.duration, "분") };
        ...
        return <div key={exercise.exerciseId} className="rounded-2xl border border-gray-100 p-4">
          <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-gray-900">{name}</p><p className="mt-1 text-xs text-[#534AB7]">AI 추천: {exercise.sets || exercise.duration || "통증 없는 범위"}</p></div><button type="button" onClick={() => onChange({ ...settings, exerciseTargets: { ...settings.exerciseTargets, [name]: ai } })} className="shrink-0 rounded-lg bg-[#EEEDFE] px-2.5 py-1.5 text-[11px] font-bold text-[#3C3489]">추천값 적용</button></div>
          ...
          <p className="mt-2 text-[11px] text-gray-400">추천 이유: 현재 주차 계획과 허리 안전 기준을 우선한 시작값입니다.</p>
        </div>;
      })}
    </div>}
  </section>;
