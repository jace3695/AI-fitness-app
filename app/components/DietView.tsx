...
export default function DietView() {
  ...
  return <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
    ...
    <section className="rounded-2xl bg-white border border-gray-100 p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2"><button onClick={saveDiet} className="rounded-xl bg-[#534AB7] px-4 py-3 text-[14px] font-bold text-white">식단 기록 저장</button><button onClick={cancelDiet} className="rounded-xl bg-red-50 px-4 py-3 text-[14px] font-bold text-red-600">식단 기록 취소</button></div>
      {message && <p className="mt-2 rounded-xl bg-green-50 px-3 py-2 text-[12px] font-semibold text-green-700">{message}</p>}
    </section>

    <section className="rounded-2xl bg-white border border-gray-100 p-4 shadow-sm sm:p-5 lg:col-span-2">
