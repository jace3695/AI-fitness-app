...
export default function DevicePinPanel() {
  ...
  const savePin = async (event: FormEvent) => {
    ...
    setNewPin("");
    setConfirmPin("");
    setMessage(enabled ? "공통 PIN을 변경했습니다." : "AI 연이 공통 PIN을 설정했습니다.");
    setSaving(false);
  };
  ...
  const disablePin = async () => {
    ...
    setCurrentPin("");
    setMessage("AI 연이 공통 PIN을 해제했습니다.");
    setSaving(false);
  };
  ...
  return <section className="mb-4 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="mt-1 flex items-start justify-between gap-3">
      <div><h2 className="text-lg font-bold text-gray-900">AI 연이 공통 PIN</h2><p className="mt-1 text-xs leading-relaxed text-gray-500">가계·운동·식단·언어 기능에서 함께 사용하는 6자리 잠금입니다. PIN 원문은 저장하지 않습니다.</p></div>
