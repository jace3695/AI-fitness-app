...
export default function FitnessAiCoachPanel({ stores, mode = "full", onPlanApplied, managedCircuit = false }: { stores?: RecordStores; mode?: "full" | "plan"; onPlanApplied?: (settings: UserWorkoutSettings) => void; managedCircuit?: boolean }) {
  ...
  return (
    <section className="rounded-3xl border border-violet-100 bg-gradient-to-br from-white to-[#F3F1FF] p-4 shadow-sm sm:p-5">
      <div><p className="text-[12px] font-bold text-[#534AB7]">AI 연이 운동 코치</p><h2 className="mt-1 text-[20px] font-extrabold text-gray-900">{mode === "plan" ? "내 운동계획을 먼저 점검해요" : "분석하고 다음 운동계획도 제안해요"}</h2><p className="mt-1 text-[11px] leading-5 text-gray-500">현재는 기록을 계산한 무료 점검과 계획을 보여줍니다. 제안을 확인하고 선택한 경우에만 운동 설정이 바뀝니다.</p></div>
