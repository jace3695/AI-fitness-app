...
export default function DayView({ day, isCompleted, onSaveWorkout: persistWorkout, onCancelWorkout, workoutPain = false, workoutMemo = '', workoutStatus = 'completed', workoutDifficulty = 'moderate', workoutFatigue = 2, workoutExerciseRecords = EMPTY_EXERCISE_RECORDS, cardioDone = false, cardioType = '슬라이딩보드', cardioMinutes = 15, cardioMemo = '', onSaveCardio, onCancelCardio, foamRollerDone = false, foamRollerTiming = 'before', foamRollerAreas = [], foamRollerPain = false, foamRollerMemo = '', onSaveFoamRoller, onCancelFoamRoller, onPullupTraining, recovery, onRecordRecovery, onCancelRecovery, showBaseRoutine = true, onShowRecommended, onShowBaseRoutine }: DayViewProps) {
  ...
  const onSaveWorkout: DayViewProps["onSaveWorkout"] = (painValue, memoValue, cardioOptionId, exerciseRecords, minutes, feedback) => persistWorkout(painValue, memoValue, cardioOptionId, exerciseRecords || checklistRecords, minutes, feedback);
  useEffect(() => { setPain(workoutPain); setMemo(workoutMemo); setOverallStatus(workoutStatus); setDifficulty(workoutDifficulty); setFatigue(workoutFatigue); setChecklistRecords(workoutExerciseRecords); }, [workoutPain, workoutMemo, workoutStatus, workoutDifficulty, workoutFatigue, workoutExerciseRecords]);
  ...
  const saveWorkoutResult = () => {
    ...
    onSaveWorkout(pain, memo, selectedCardioOptionId, checklistRecords, selectedCardioMinutes, { status: overallStatus, difficulty, fatigue });
  ...
  return <div>
    ...
    <details className="mb-4 rounded-2xl border border-[#D9D6FF] bg-white shadow-sm">
      ...
      <div className="px-4 pb-4"><DailyExerciseChecklist key={`${day.id}:${exercises.map((exercise) => exercise.name).join("|")}:${JSON.stringify(workoutExerciseRecords)}`} exerciseNames={exercises.map((exercise) => exercise.name)} savedRecords={workoutExerciseRecords} onChange={setChecklistRecords} /></div>
    </details>
