...
export const exerciseGuides: Record<string, ExerciseGuideEntry> = {
  ...
  '덤벨 고블릿 스쿼트': { summary: '가벼운 덤벨로 하체와 엉덩이를 강화', purpose: '하체 근력, 엉덩이 활성화, 무릎/골반 안정화', setup: ['덤벨을 가슴 앞에 세워 잡습니다.', '발은 어깨너비로 두고 발끝은 약간 바깥을 향합니다.'], movement: ['엉덩이를 뒤로 보내며 천천히 앉습니다.', '무릎이 안쪽으로 모이지 않게 합니다.', '발바닥 전체로 밀어 올라옵니다.'], breathing: '내려갈 때 들이마시고 올라올 때 내쉽니다.', target: '허벅지, 엉덩이, 코어', commonMistakes: ['무릎이 안으로 모임', '허리가 말림'], stopCriteria: ['무릎 통증이 있으면 범위를 줄이세요.', '허리가 불편하면 무게를 낮추세요.', '자세가 무너지면 중단하세요.'], alternatives: ['덤벨 없이 의자 스쿼트', '힙브릿지'], keyPoint: '5~7kg부터 시작하고 자세가 먼저입니다.' },
