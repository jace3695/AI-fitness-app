...
export function workoutTimeHistory(rows:unknown,owner:string,today:string,meals:Record<string,unknown>) {
  ...
  const days=(rows as WorkoutTime[]).map(row=>{
    ...
    const clock=meal&&typeof meal==='object'&&!Array.isArray(meal)?(meal as Record<string,unknown>).lastMealTime:undefined;
    return {date:row.recorded_on,start:row.starts_at,end:row.ends_at,minutes:clockMinutes(row.ends_at)!-clockMinutes(row.starts_at)!,meal:clockMinutes(clock)===null?null:clock as string,gap:workoutMealGap(row,clock)};
