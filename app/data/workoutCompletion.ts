...
export type WorkoutDayId = 'mon' | 'tue' | 'thu' | 'fri' | 'sat';
...
export const WORKOUT_DAY_IDS: WorkoutDayId[] = ['mon', 'tue', 'thu', 'fri', 'sat'];

const dayIndexById: Record<WorkoutDayId, number> = { mon: 1, tue: 2, thu: 4, fri: 5, sat: 6 };
...
export function getWeeklyWorkoutCompletion(store: WorkoutCompletionStore, baseDate = new Date()) {
  return WORKOUT_DAY_IDS.reduce<Record<WorkoutDayId, boolean>>((acc, dayId) => {
    ...
    return acc;
  }, { mon: false, tue: false, thu: false, fri: false, sat: false });
}
