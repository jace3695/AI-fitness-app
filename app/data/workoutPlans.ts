...
export const DEFAULT_WEEKLY_WORKOUT_PLAN_ID = 'five-day-fullbody-circuit';

export const WEEKLY_WORKOUT_PLANS: WeeklyWorkoutPlan[] = [
  { id: 'five-day-fullbody-circuit', name: '주 5일 전신 서킷 — 근력·회복 교차', description: '월·수·금은 전신 근력, 화·목은 낮은 강도의 전신·코어 회복 서킷으로 피로를 조절합니다.', recommendedFor: '체지방을 줄이면서 근육과 운동능력을 유지하고 몸통 안정성을 기르려는 현재 사용자', weekLabel: '현재 기본 계획', notice: '슬라이딩보드는 운동 라이브러리에 유지되지만 현재 개인 루틴에서는 제외됩니다. 서킷은 속도 경쟁이 아니며 자세가 흐트러지거나 숨이 너무 차면 동작 사이 20~40초, 라운드 사이 60~90초 쉽니다.', days: { monday: 'current-fullbody-strength-circuit', tuesday: 'current-fullbody-recovery-circuit', wednesday: 'current-fullbody-strength-circuit', thursday: 'current-fullbody-recovery-circuit', friday: 'current-fullbody-strength-circuit', saturday: 'current-weekend-recovery', sunday: 'rest' } },
