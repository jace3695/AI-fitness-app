...
export const SAFETY_STOP_MESSAGE = '허리 통증, 다리 저림, 무릎 통증, 어지럼, 손떨림, 식은땀, 메스꺼움, 가슴 답답함이 있으면 즉시 중단하고 회복일로 전환하세요.';

const flow: FlowItem[] = [
  { icon:'🧘', label:'워밍업', time:'5~8분', bgColor:'#EEEDFE', labelColor:'#3C3489', timeColor:'#534AB7' },
  { icon:'💪', label:'본운동', time:'15~25분', bgColor:'#E6F1FB', labelColor:'#0C447C', timeColor:'#185FA5' },
...
const phases = (exercises:Exercise[], alert?:string): Phase[] => [
  ...
  { id:'main', icon:'💪', title:'본운동', subtitle:'15~25분 기준 · 처음 2주는 세트 수와 무게를 늘리지 않음', alert: alert ? {variant:'yellow', text:alert} : undefined, exercises },
...
const day = (id:string, tabLabel:string, emoji:string, title:string, subtitle:string, exercises:Exercise[], color:string, alert?:string): DayWorkout => ({ id, tabLabel, emoji, title, subtitle, totalTime:'본운동 15~25분', badgeBg:color, dayColor:color, flow, phases:phases(exercises, alert) });

export const ADAPTATION_WORKOUTS: Record<AdaptationRoutineSelection, DayWorkout> = {
  adapt1: day('adapt1','운동 A','🌱','운동 A — 허리 안정화 + 등','총 15~20분 · 월요일 권장 루틴',[
    mk('버드독','좌우 6회 × 2세트 · 휴식 30초','골반이 흔들리지 않게 천천히 뻗습니다.',2,30),
    mk('힙 브릿지','12회 × 2세트 · 휴식 30초','허리가 아니라 엉덩이에 힘을 줍니다.',2,30),
    mk('롱밴드 로우','12회 × 2세트 · 휴식 45초','팔보다 등을 먼저 조인다는 느낌으로 당깁니다.',2,45),
    mk('데드버그','좌우 6회 × 2세트 · 휴식 30초','허리가 뜨지 않는 범위에서 팔다리를 천천히 움직입니다.',2,30),
    sliding('가벼운 슬라이딩보드','5분','숨은 차지만 대화 가능한 정도로만 진행합니다.',{segments:[{label:'가볍게',seconds:300,intensity:'가볍게'}]})
  ], '#534AB7'),
  adapt2: day('adapt2','운동 B','🌿','운동 B — 하체 + 골반 안정','총 15~20분 · 수요일 권장 루틴',[
    mk('의자 스쿼트','10회 × 2세트 · 휴식 45초','의자에 가볍게 앉았다 일어나며 무릎과 허리 통증 없는 범위만 진행합니다.',2,45),
    mk('힙 브릿지','12회 × 2세트 · 휴식 30초','엉덩이를 조이고 허리를 과하게 젖히지 않습니다.',2,30),
    mk('루프밴드 사이드 워크','좌우 10걸음 × 2세트 · 휴식 30초','작은 보폭으로 골반 높이를 유지합니다.',2,30),
    mk('버드독','좌우 6회 × 2세트 · 휴식 30초','흔들림 없는 자세를 우선합니다.',2,30),
    sliding('가벼운 슬라이딩보드','5분','무릎을 살짝 굽히고 균일하게 미끄러집니다.',{segments:[{label:'가볍게',seconds:300,intensity:'가볍게'}]})
  ], '#639922'),
  adapt3: day('adapt3','운동 C','⚡','운동 C — 상체 + 코어 안정화','총 15~20분 · 금요일 권장 루틴',[
    mk('롱밴드 랫풀다운','12회 × 2세트 · 휴식 45초','팔꿈치를 아래·뒤로 당기며 어깨를 귀에서 멀리 둡니다.',2,45),
    mk('덤벨 플로어 프레스','10회 × 2세트 · 휴식 45초','한 손 3~5kg부터 시작하고 무게 욕심을 내지 않습니다.',2,45),
    mk('루프밴드 풀어파트','12~15회 × 2세트 · 휴식 30초','어깨를 내리고 등 위쪽으로 밴드를 벌립니다.',2,30),
    mk('무릎 사이드 플랭크','좌우 10~15초 × 2세트 · 휴식 30초','오래 버티지 말고 허리 통증 없는 짧은 자세만 유지합니다.',2,30),
    sliding('가벼운 슬라이딩보드','5분','마무리 유산소로 가볍게 진행합니다.',{segments:[{label:'가볍게',seconds:300,intensity:'가볍게'}]})
  ], '#EF9F27')
};

export const WORKOUTS: DayWorkout[] = [
  day('mon','월요일','💪','월요일 — 운동 A: 허리 안정화 + 등','총 15~20분 · 턱걸이 초기자세 3분 포함', ADAPTATION_WORKOUTS.adapt1.phases[1].exercises, '#534AB7'),
  day('tue','화요일','🚶','화요일 — 가벼운 유산소 + 턱걸이 초기자세','가벼운 유산소 15~20분 중심', [mk('가벼운 유산소','15~20분','걷기 대신 슬라이딩보드, 실내 걷기, 고정식 자전거 등으로 대체할 수 있습니다.',0,0), mk('턱걸이 초기자세 연습','3분','최대 버티기가 아니라 철봉 잡기·발 보조 매달리기·견갑 내리기 감각만 짧게 연습합니다.',0,0)], '#378ADD'),
  day('wed','수요일','🌿','수요일 — 운동 B: 하체 + 골반 안정','총 15~20분 · 턱걸이 초기자세 3분 포함', ADAPTATION_WORKOUTS.adapt2.phases[1].exercises, '#639922'),
  day('thu','목요일','🚶','목요일 — 가벼운 유산소 + 턱걸이 초기자세','가벼운 유산소 15~20분 중심', [mk('가벼운 유산소','15~20분','걷기 대신 슬라이딩보드, 실내 걷기, 고정식 자전거 등으로 대체할 수 있습니다.',0,0), mk('턱걸이 초기자세 연습','3분','반동 없이 어깨를 귀에서 멀리 두는 감각만 확인합니다.',0,0)], '#378ADD'),
  day('fri','금요일','⚡','금요일 — 운동 C: 상체 + 코어 안정화','총 15~20분 · 턱걸이 초기자세 3분 포함', ADAPTATION_WORKOUTS.adapt3.phases[1].exercises, '#EF9F27'),
  day('sat','토요일','🌤️','토요일 — 선택 유산소 + 턱걸이 초기자세','선택 유산소 10~20분 중심', [sliding('선택 유산소','10~20분','슬라이딩보드를 1순위로 추천하고, 실내 걷기·야외 걷기·고정식 자전거·기타 가벼운 유산소 중 선택합니다.',{segments:[{label:'가볍게',seconds:600,intensity:'가볍게'}]}), mk('턱걸이 초기자세 연습','3분','짧은 자세 연습만 진행합니다.',0,0)], '#E24B4A'),
  day('sun','일요일','😴','일요일 — 휴식 또는 턱걸이 자세만 1~2분','회복 중심 · 필요 시 짧은 자세 확인만 진행', [mk('휴식','권장','몸 상태를 회복하고 다음 주 운동을 준비합니다.',0,0), mk('턱걸이 자세 확인','1~2분','통증이 없을 때만 철봉 잡기와 견갑 내리기 감각을 짧게 확인합니다.',0,0)], '#9CA3AF'),
];

export const WEEK_OVERVIEW = {
  stats:[{label:'근력/안정화',value:'3',unit:'일',sub:'월·수·금'},{label:'본운동',value:'15~25',unit:'분',sub:'처음 2주'},{label:'선택 유산소',value:'3~5',unit:'회',sub:'화·목·토 선택'},{label:'턱걸이 초기자세',value:'매일',unit:'',sub:'3~5분'}],
  days:[{day:'월',emoji:'💪',label:'운동 A',sub:'허리·등',time:'15~20분',active:true,tabId:'mon',color:'#534AB7',bg:'#EEEDFE',border:'#AFA9EC'},{day:'화',emoji:'🚶',label:'가벼운 유산소',sub:'자세 3분',time:'15~20분',active:true,tabId:'tue',color:'#378ADD',bg:'#E6F1FB',border:'#9CCAF0'},{day:'수',emoji:'🌿',label:'운동 B',sub:'하체·골반',time:'15~20분',active:true,tabId:'wed',color:'#639922',bg:'#EAF3DE',border:'#B7D88B'},{day:'목',emoji:'🚶',label:'가벼운 유산소',sub:'자세 3분',time:'15~20분',active:true,tabId:'thu',color:'#378ADD',bg:'#E6F1FB',border:'#9CCAF0'},{day:'금',emoji:'⚡',label:'운동 C',sub:'상체·코어',time:'15~20분',active:true,tabId:'fri',color:'#EF9F27',bg:'#FAEEDA',border:'#F3C276'},{day:'토',emoji:'🌤️',label:'선택',sub:'유산소 선택',time:'10~20분',active:true,tabId:'sat',color:'#E24B4A',bg:'#FCEBEB',border:'#F1A6A6'},{day:'일',emoji:'😴',label:'휴식',sub:'자세 1~2분',time:'휴식',active:true,tabId:'sun',color:'#6B7280',bg:'#F3F4F6',border:'#D1D5DB'}],
  dayTimes:[{days:'월',total:'15~20분',detail:'운동 A',color:'#534AB7',border:'#AFA9EC'},{days:'화',total:'15~20분',detail:'가벼운 유산소',color:'#378ADD',border:'#9CCAF0'},{days:'수',total:'15~20분',detail:'운동 B',color:'#639922',border:'#B7D88B'},{days:'목',total:'15~20분',detail:'가벼운 유산소',color:'#378ADD',border:'#9CCAF0'},{days:'금',total:'15~20분',detail:'운동 C',color:'#EF9F27',border:'#F3C276'},{days:'토',total:'10~20분',detail:'선택 유산소',color:'#E24B4A',border:'#F1A6A6'},{days:'일',total:'휴식',detail:'자세 1~2분',color:'#6B7280',border:'#D1D5DB'}]
};
export const SAFETY = { forbidden:'처음 2주는 숨은 차지만 대화 가능한 정도로 진행합니다. 운동 후 허리 통증이 없어야 하며 다음날 피로가 심하면 강도가 과한 것입니다. 세트 수는 늘리지 않고 덤벨 무게 욕심을 내지 않습니다. AB 슬라이드, 윗몸일으키기, 레그레이즈, 러시안 트위스트, 무거운 데드리프트, 백익스텐션, 점프 운동, 빠른 버피, 무리한 플랭크 오래 버티기는 제외합니다.', signals:[{label:'가볍게',action:'대화 가능',bg:'#EAF3DE',labelColor:'#3B6D11',textColor:'#27500A'},{label:'중간',action:'짧은 문장은 가능하지만 숨이 참',bg:'#E6F1FB',labelColor:'#185FA5',textColor:'#0C447C'},{label:'강하게',action:'긴 대화가 어려울 정도. 자세는 유지',bg:'#FAEEDA',labelColor:'#854F0B',textColor:'#633806'}], cpap: SAFETY_STOP_MESSAGE };
