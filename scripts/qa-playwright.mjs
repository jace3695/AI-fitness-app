...
child.on('close', code => {
  ...
  writeFileSync(`.e2e/evidence/browser-environment-${invocation}.json`, JSON.stringify({ commit: process.env.QA_HEAD_SHA,
  ...
  writeFileSync(`.e2e/evidence/native-process-${invocation}.json`, JSON.stringify({ commit: process.env.QA_HEAD_SHA, dropped, nativeCounts, events: native }, null, 2));
  try {
    const report = JSON.parse(readFileSync('.e2e/evidence/results.json', 'utf8'));
    console.log('QA_BROWSER_RESULT ' + JSON.stringify({ commit: report.commit, status: report.status,
      passed: report.results.filter(row => row.status === 'passed').length,
      failed: report.results.filter(row => row.status !== 'passed').length }));
  } catch { console.error('Verification result missing'); process.exitCode = 1; }
