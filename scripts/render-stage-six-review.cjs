...
const {StructureDiagram} = load('components/drawing/StructurePractice.tsx');
...
const dir='docs/drawing/stage-six';fs.mkdirSync(dir,{recursive:true});
(async()=>{
 for(const lesson of pack.lessons.slice(42,52)) for(const [i,example] of lesson.examples.entries()) {
  const doc=newDocument(lesson,example,pack.version);
  const panels=[['Original',svg(Diagram,{example,original:true})],['Structure / hidden parts',svg(StructureDiagram,{doc,mode:'full'})],['Easy guide',svg(StructureDiagram,{doc,mode:'easy'})],...lesson.steps.map((s,j)=>[`Action ${j+1}`,svg(StructureDiagram,{doc:{...doc,step:j},mode:'demo'})]),...example.structure.choices.map(c=>[`Choice: ${c.id}`,svg(StructureDiagram,{doc,mode:'easy',choice:c.id})])];
 ...
 console.log('20 structure review sheets');
})();
