...
const lessons = pack.lessons.slice(1,8);
...
const dir='docs/drawing/stage-one';mkdirSync(dir,{recursive:true});
 ...
 for (const [ei,ex] of lesson.examples.entries()) {
  ...
  for (const [si,step] of lesson.steps.entries()) {
   ...
   for (const l of ex.lines) {
    const active=step.lines.includes(l.id), current=active&&step.action!=='look';
   ...
   svg+=text(step.text,400,16)+'</g>';if(step.action==='draw')step.lines.forEach(id=>seen.add(id));
  }
  const i=li*2+ei;
  finals+=`<g transform="translate(${i%4*400},${Math.floor(i/4)*400})">`+text(`${lesson.id} · ${ex.name}`,24)+ex.lines.map(l=>`<path d="${l.d}" fill="${l.fill==='ink'?'#161616':'none'}" stroke="${l.fill==='ink'?'none':'#161616'}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>`).join('')+'</g>';
 }
...
console.log('D02–D08: 14 final examples and 82 step panels rendered with exact lesson text.');
