...
begin
  ...
  select coalesce(sum(cost_krw), 0) into v_spent from public.ai_usage_events
   where user_id = p_user_id and created_at >= date_trunc('month', now());
  if v_spent + greatest(p_estimated_cost_krw, 0.01) > v_limit then
    return query select null::uuid, v_spent, greatest(v_limit - v_spent, 0);
    return;
  end if;
  insert into public.ai_usage_events(user_id, provider, model, feature, usage_kind, cost_krw)
  values (p_user_id, p_provider, left(p_model, 80), left(p_feature, 80), p_usage_kind, greatest(p_estimated_cost_krw, 0.01))
...
create or replace function public.finalize_ai_usage(p_reservation_id uuid, p_input_units bigint, p_output_units bigint, p_actual_cost_krw numeric)
returns void language plpgsql security definer set search_path = '' as $$
begin
  update public.ai_usage_events set
    input_units = greatest(p_input_units, 0), output_units = greatest(p_output_units, 0),
    cost_krw = least(cost_krw, greatest(p_actual_cost_krw, 0.01)), is_reservation = false, finalized_at = now()
  where id = p_reservation_id and user_id = auth.uid() and is_reservation;
end; $$;

create or replace function public.cancel_ai_budget_reservation(p_reservation_id uuid)
returns void language sql security definer set search_path = '' as $$
  delete from public.ai_usage_events where id = p_reservation_id and user_id = auth.uid() and is_reservation;
$$;

revoke all on function public.reserve_ai_budget(uuid,text,text,text,numeric,text) from public, anon;
revoke all on function public.finalize_ai_usage(uuid,bigint,bigint,numeric) from public, anon;
revoke all on function public.cancel_ai_budget_reservation(uuid) from public, anon;
grant execute on function public.reserve_ai_budget(uuid,text,text,text,numeric,text) to authenticated;
grant execute on function public.finalize_ai_usage(uuid,bigint,bigint,numeric) to authenticated;
grant execute on function public.cancel_ai_budget_reservation(uuid) to authenticated;
